...
app.post('/api/report', async (req, res) => {
    try {
        ...
        const sessionDir = path.join(CAPTURE_DIR, `CASE_${timestamp}`);
        ...
        if (video) {
            try {
                ...
                // Save as .webm (standard for browser recording)
        ...
        doc.text(`PLATFORM:     ${meta.userAgent}`);
        ...
        if (images && images.length > 0) {
            images.forEach((base64String, index) => {
                try {
                    ...
                    fs.writeFileSync(path.join(sessionDir, `evidence_${index + 1}.jpg`), imgBuffer);
        ...
        doc.on('end', () => {
            ...
            // Send PDF back to the browser (Redirect)
